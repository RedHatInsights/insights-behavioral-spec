Feature: Disabling rule in OCM user interface
  ...
  Scenario: Check if rule R1 is visible for two test accounts
    Given OCM user interface is accessible
      And CCX external data pipeline is in working state
      And user U1 exists for test organization
      And user U2 exists for test organization
  ...
  Scenario: Check if rule R1 disabled by first test account is disabled for second test account as well
    Given OCM user interface is accessible
      And CCX external data pipeline is in working state
      And user U2 exists for test organization
      And cluster C1 contains issue detected by rule R1
     ...
     When user selects Insights tab
     Then list of rule hits should be displayed on OCM UI
      And the rule R1 should be no longer visible in list of rule hits


  Scenario: Check if first test account is able to enable rule R1
    Given OCM user interface is accessible
      And CCX external data pipeline is in working state
      And user U1 exists for test organization
      And cluster C1 contains issue detected by rule R1
     ...
     Then list of rule hits should be displayed on OCM UI
      And the rule R1 should be no longer visible in list of rule hits
     When user enables rule R1 by using UI widget
      And the rule R1 should be visible in list of rule hits


  Scenario: Check if rule R1 enabled by first test account is enabled for second test account as well
    Given OCM user interface is accessible
      And CCX external data pipeline is in working state
      And user U2 exists for test organization
      And cluster C1 contains issue detected by rule R1
