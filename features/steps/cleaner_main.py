...
from src.process_output import filepath_from_context, process_generated_output
...
def run_cleaner_for_older_records(context, age):
    ...
    stdout_file = filepath_from_context(
        context, "logs/insights-results-aggregator-cleaner/", "_stdout")
    stderr_file = filepath_from_context(
        context, "logs/insights-results-aggregator-cleaner/", "_stderr")

    # it is expected that exit code will be 0
    process_generated_output(context, out, 0, stdout_file=stdout_file,
...
def run_cleanup_all_no_dry_run(context, age):
    """Negate the -dry-run parameter as it's true by default."""
