...
@when(
    "I run the Insights Results Aggregator with the {flag} command line flag and config file name set to {config}"  # noqa: E501
)
...
@then(
    "I should see actual configuration displayed by Insights Results Aggregator on standard output"
)  # noqa E501
...
def start_insights_results_aggregator_in_background(context):
    ...
    if hasattr(context, "aggregator_process") and context.aggregator_process.poll() is None:
        return
...
@when(
    "I access endpoint {endpoint} using HTTP GET method using token for organization {organization:d} account number {account}, and user {user}"  # noqa: E501
)
...
def check_empty_list_of_clusters(context):
    ...
    # check if the list is empty
    assert len(found_clusters) == 0, f"Expected no clusters but {found_clusters} has been returned"


@when(
    "I ask for list of all disabled rules for organization {organization:d} account number {account}, and user {user}"  # noqa E501
)
...
@when(
    "I enable rule {rule_id} with error key {error_key} for organization {organization:d} account number {account}, and user {user}"  # noqa E501
)
...
@when(
    "I disable rule {rule_id} with error key {error_key} for organization {organization:d} account number {account} and user {user} with justification '{justification}'"  # noqa E501
)
def disable_rule_in_aggregator(
    context, rule_id, error_key, organization, account, user, justification
):  # noqa E501
...
@when(
    "I update rule {rule_id} with error key {error_key} for organization {organization:d} account number {account} and user {user} with justification '{justification}'"  # noqa E501
)
def update_rule_in_aggregator(
    context, rule_id, error_key, organization, account, user, justification
):
