...
def check_parquet_table_is(context, object_name):
    ...
    want_df = want_df.sort_values(by=["archive_path"], ascending=False).reset_index(drop=True)
    got_df = got_df.sort_values(by=["archive_path"], ascending=False).reset_index(drop=True)
