Feature: Recommendation description view in case some or all issues are disabled in Insights Advisor


  Scenario: Displaying description of recommendation found for 1 cluster with 1 critical issue and 1 other issue disabled in Insights Advisor
  ...
  Scenario: Displaying description of recommendation found for 1 cluster with 1 critical issue and 1 other issue being enabled in Insights Advisor
    Given user USER1 is part of account (organization) ACCOUNT1
      And account (organization) ACCOUNT1 owns one managed cluster
      And the infrastructure for that managed cluster is provided by Amazon Web Services
      And that managed cluster should be in Ready state
      And 1 critical issue is detected for this cluster: PROMETHEUS_DB_VOLUME_IS_EMPTY
      And 1 other critical issue #2 is detected for this cluster
      And issue #2 is disabled in Insights Advisor
      And the user USER1 is already logged in into Advanced Cluster Management
     When user looks at Advanced Cluster Management main page
     Then menu on the left side should be displayed
  ...
  Scenario: Displaying description of recommendation found for 1 cluster with 2 critical issue, one issue being disabled in Insights Advisor
    Given user USER1 is part of account (organization) ACCOUNT1
      And account (organization) ACCOUNT1 owns one managed cluster
      And the infrastructure for that managed cluster is provided by Amazon Web Services
      And that managed cluster should be in Ready state
     ...
     Then the donut chart should display "2" in the centre
      And should contain just red arc
     When the issue #2 is disabled in Insights Advisor
      And I look into "Cluster issues" section
